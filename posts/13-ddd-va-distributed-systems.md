...
Vì sao phải tách? Vì **event schema chính là API**. Khi bạn publish domain event nội bộ ra broker, mọi field của aggregate trở thành contract công khai: sáu tháng sau bạn đổi tên field `totalAmount` → `grandTotal` trong lúc refactor domain model — và ba consumer của ba team khác chết cùng lúc, dù bạn "chỉ refactor nội bộ". Bạn đã vô tình biến domain model — thứ cần tự do tiến hóa nhất — thành thứ bị đóng băng bởi các team khác. Đây chính xác là mất mát mà bounded context sinh ra để tránh ([Chương 4](#/post/04-bounded-context)).

Cách làm đúng: domain event sinh ra trong aggregate → một translator ở tầng application/infrastructure của context map nó sang integration event (một class/struct riêng, version tường minh) → publish. Translator này chính là **Anti-Corruption Layer chiều đi** ([Chương 5](#/post/05-context-mapping)).
